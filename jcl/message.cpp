...
void Message::treatMessage() {
    ...
    switch (m_jcl->message[typePosition]) {
        ...
        case 51: {   // Acting
            if (m_jcl->get_metadata()->is_standBy()) {
                ...
                if (!Sensor::validPin(pin))
                    sendResultBool(false);
                if (m_jcl->get_sensors()[pin] == NULL || m_jcl->get_sensors()[pin]->get_typeIO() == Constants::CHAR_INPUT)
                    sendResultBool(false);
        ...
        case 53: {   // set_encryption
            if (m_jcl->get_metadata()->is_standBy()) {
                sendResultBool(false);
            } else {
                // printMessagePROGMEM(Constants::setEncryptionMessage);
                Serial.println("set_encryption");
...
/*for (int jj=0; jj<nChars; jj++){
  Serial.print((int) act->get_param()[jj]);
  Serial.print("  ");
  Serial.print((char) act->get_param()[jj]);
  Serial.println();
}
...
bool Message::messageSetSensor() {
    ...
    if (m_jcl->get_sensors()[atoi(s->get_pin())] != NULL) {
        m_jcl->get_sensors()[atoi(s->get_pin())]->deleteSensor();
        m_jcl->get_sensors()[atoi(s->get_pin())] = NULL;
    } else {
        char nSensors[4];
        sprintf(nSensors, "%d", atoi(m_jcl->get_metadata()->get_numConfiguredSensors()) + 1);
        m_jcl->get_metadata()->set_numConfiguredSensors(nSensors);
    ...
    s->configurePinMode();
    m_jcl->listSensors();
//m_jcl->availableSensors[m_jcl->numSensors++] = atoi((s->get_pin()));
    return true;
}
...
void Message::sendMetadata(int messageType) {
    ...
    int sensorsSize = 0;
    if (atoi(m_jcl->get_metadata()->get_numConfiguredSensors()) != 0) {
        sensorsSize += strlen(Constants::enableSensorMessage) + enableSensorSize + 4 + 2 +
                       (strlen(Constants::sensorAliasMessage) * atoi(m_jcl->get_metadata()->get_numConfiguredSensors())) +
                       totalSizePins + totalSizeNameSensor +
                       ((4 + 2) * atoi(m_jcl->get_metadata()->get_numConfiguredSensors())) +
                       (strlen(Constants::sensorDirMessage) * atoi(m_jcl->get_metadata()->get_numConfiguredSensors())) +
                       totalSizePins + totalDirSensor +
                       ((4 + 2) * atoi(m_jcl->get_metadata()->get_numConfiguredSensors())) +
                       (strlen(Constants::sensorSizeMessage) * atoi(m_jcl->get_metadata()->get_numConfiguredSensors())) +
                       totalSizePins + totalSensorSize +
                       ((4 + 2) * atoi(m_jcl->get_metadata()->get_numConfiguredSensors())) +
                       (strlen(Constants::sensorSamplingMessage) *
                        atoi(m_jcl->get_metadata()->get_numConfiguredSensors())) + totalSizePins + totalTimeSensor +
                       ((4 + 2) * atoi(m_jcl->get_metadata()->get_numConfiguredSensors()));
    } else
        sensorsSize += strlen(Constants::enableSensorMessage) + enableSensorSize + 4 + 2;

    int standBySize = m_jcl->get_metadata()->is_standBy() ? strlen(Constants::trueMessage) : strlen(
            Constants::falseMessage);
    // Calculate size to append to the message
    totalSize =
            strlen(Constants::portMessage) + strlen(m_jcl->get_metadata()->get_hostPort()) + 4 + 2 +
            ...
            strlen(Constants::deviceIDMessage) + strlen(m_jcl->get_metadata()->get_boardName()) + 4 + 2 +
            strlen(Constants::macMessage) + strlen(m_jcl->get_metadata()->get_mac()) + 4 + 2 +
            strlen(Constants::Constants::devicePlatformMessage) + strlen(Constants::devicePlatformValue) + 4 + 2 +
            strlen(Constants::numberSensorsMessage) + strlen(m_jcl->get_metadata()->get_numConfiguredSensors()) + 4 + 2 +
            strlen(Constants::standByMessage) + standBySize + 4 + 2 +
            sensorsSize;

    if (messageType == -1) {
        totalSize += strlen(Constants::ipMessage) + strlen(m_jcl->get_metadata()->get_hostIP()) + 4 + 2;
    }
    ...
    m_jcl->message[currentPosition++] =
            strlen(Constants::deviceIDMessage) + strlen(m_jcl->get_metadata()->get_boardName()) + 4;
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::deviceIDMessage);
    for (unsigned int i = 0; i < strlen(Constants::deviceIDMessage); i++)
        m_jcl->message[currentPosition++] = Constants::deviceIDMessage[i];
    m_jcl->message[currentPosition++] = 18;
    m_jcl->message[currentPosition++] = strlen(m_jcl->get_metadata()->get_boardName());
    for (unsigned int i = 0; i < strlen(m_jcl->get_metadata()->get_boardName()); i++)
    ...
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::portMessage) + strlen(m_jcl->get_metadata()->get_hostPort()) + 4;
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::portMessage);
    ...
    if (messageType == -1) {
        ...
        m_jcl->message[currentPosition++] = 10;
        m_jcl->message[currentPosition++] = strlen(Constants::ipMessage) + strlen(m_jcl->get_metadata()->get_hostIP()) + 4;
        m_jcl->message[currentPosition++] = 10;
        m_jcl->message[currentPosition++] = strlen(Constants::ipMessage);
    ...
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::coreMessage) + strlen(Constants::coreValue) + 4;
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::coreMessage);
    for (unsigned int i = 0; i < strlen(Constants::coreMessage); i++)
        m_jcl->message[currentPosition++] = Constants::coreMessage[i];
    m_jcl->message[currentPosition++] = 18;
    m_jcl->message[currentPosition++] = strlen(Constants::coreValue);
    ...
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::macMessage) + strlen(m_jcl->get_metadata()->get_mac()) + 4;
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::macMessage);
    ...
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::devicePlatformMessage);
    ...
    m_jcl->message[currentPosition++] = 18;
    m_jcl->message[currentPosition++] = strlen(Constants::devicePlatformValue);
    ...
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] =
            strlen(Constants::numberSensorsMessage) + strlen(m_jcl->get_metadata()->get_numConfiguredSensors()) + 4;
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::numberSensorsMessage);
    ...
    m_jcl->message[currentPosition++] = 18;
    m_jcl->message[currentPosition++] = strlen(m_jcl->get_metadata()->get_numConfiguredSensors());
    for (unsigned int k = 0; k < strlen(m_jcl->get_metadata()->get_numConfiguredSensors()); k++)
        m_jcl->message[currentPosition++] = m_jcl->get_metadata()->get_numConfiguredSensors()[k];

    /* Setting the value of the STANDBY */
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::standByMessage) + standBySize + 4;
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::standByMessage);
    for (unsigned int k = 0; k < strlen(Constants::standByMessage); k++)
        m_jcl->message[currentPosition++] = Constants::standByMessage[k];
    m_jcl->message[currentPosition++] = 18;
    if (m_jcl->get_metadata()->is_standBy()) {
        m_jcl->message[currentPosition++] = strlen(Constants::trueMessage);
        for (unsigned int k = 0; k < strlen(Constants::trueMessage); k++)
            m_jcl->message[currentPosition++] = Constants::trueMessage[k];
    } else {
        m_jcl->message[currentPosition++] = strlen(Constants::falseMessage);
...
//  if (atoi(m_jcl->get_metadata()->get_numConfiguredSensors()) != 0){
    ...
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::enableSensorMessage) + enableSensorSize + 4;
    m_jcl->message[currentPosition++] = 10;
    m_jcl->message[currentPosition++] = strlen(Constants::enableSensorMessage);
    ...
    for (i = 0; i < JCL::get_totalSensors(); i++) {
        if (m_jcl->get_sensors()[i] != NULL) {
            m_jcl->message[currentPosition++] = 10;
            m_jcl->message[currentPosition++] =
                    strlen(Constants::sensorAliasMessage) + strlen(m_jcl->get_sensors()[i]->get_pin()) +
                    strlen(m_jcl->get_sensors()[i]->get_sensorNickname()) + 4;
            m_jcl->message[currentPosition++] = 10;
            m_jcl->message[currentPosition++] =
                    strlen(Constants::sensorAliasMessage) + strlen(m_jcl->get_sensors()[i]->get_pin());
            for (unsigned int k = 0; k < strlen(Constants::sensorAliasMessage); k++)
                m_jcl->message[currentPosition++] = Constants::sensorAliasMessage[k];
            for (unsigned int k = 0; k < strlen(m_jcl->get_sensors()[i]->get_pin()); k++)
                m_jcl->message[currentPosition++] = (char) m_jcl->get_sensors()[i]->get_pin()[k];
            m_jcl->message[currentPosition++] = 18;
            m_jcl->message[currentPosition++] = strlen(m_jcl->get_sensors()[i]->get_sensorNickname());
            for (unsigned int k = 0; k < strlen(m_jcl->get_sensors()[i]->get_sensorNickname()); k++)
    ...
    for (i = 0; i < JCL::get_totalSensors(); i++) {
        if (m_jcl->get_sensors()[i] != NULL) {
            m_jcl->message[currentPosition++] = 10;
            m_jcl->message[currentPosition++] =
                    strlen(Constants::sensorSizeMessage) + strlen(m_jcl->get_sensors()[i]->get_pin()) +
                    strlen(m_jcl->get_sensors()[i]->get_sensorSize()) + 4;
            m_jcl->message[currentPosition++] = 10;
            m_jcl->message[currentPosition++] =
                    strlen(Constants::sensorSizeMessage) + strlen(m_jcl->get_sensors()[i]->get_pin());
            ...
            for (unsigned int k = 0; k < strlen(m_jcl->get_sensors()[i]->get_pin()); k++)
                m_jcl->message[currentPosition++] = (char) m_jcl->get_sensors()[i]->get_pin()[k];
            m_jcl->message[currentPosition++] = 18;
            m_jcl->message[currentPosition++] = strlen(m_jcl->get_sensors()[i]->get_sensorSize());
            for (unsigned int k = 0; k < strlen(m_jcl->get_sensors()[i]->get_sensorSize()); k++)
    ...
    for (i = 0; i < JCL::get_totalSensors(); i++) {
        if (m_jcl->get_sensors()[i] != NULL) {
            m_jcl->message[currentPosition++] = 10;
            m_jcl->message[currentPosition++] =
                    strlen(Constants::sensorDirMessage) + strlen(m_jcl->get_sensors()[i]->get_pin()) + 1 + 4;
            m_jcl->message[currentPosition++] = 10;
            m_jcl->message[currentPosition++] =
                    strlen(Constants::sensorDirMessage) + strlen(m_jcl->get_sensors()[i]->get_pin());
    ...
    for (i = 0; i < JCL::get_totalSensors(); i++) {
        if (m_jcl->get_sensors()[i] != NULL) {
            m_jcl->message[currentPosition++] = 10;
            m_jcl->message[currentPosition++] =
                    strlen(Constants::sensorSamplingMessage) + strlen(m_jcl->get_sensors()[i]->get_pin()) +
                    strlen(m_jcl->get_sensors()[i]->get_delay()) + 4;
            m_jcl->message[currentPosition++] = 10;
            m_jcl->message[currentPosition++] =
                    strlen(Constants::sensorSamplingMessage) + strlen(m_jcl->get_sensors()[i]->get_pin());
            ...
            for (unsigned int k = 0; k < strlen(m_jcl->get_sensors()[i]->get_pin()); k++)
                m_jcl->message[currentPosition++] = (char) m_jcl->get_sensors()[i]->get_pin()[k];
            m_jcl->message[currentPosition++] = 18;
            m_jcl->message[currentPosition++] = strlen(m_jcl->get_sensors()[i]->get_delay());
            for (unsigned int k = 0; k < strlen(m_jcl->get_sensors()[i]->get_delay()); k++)
...
//Serial.print("sensing: "); Serial.println(pin);
    ...
    m_jcl->message[currentPosition++] = strlen(m_jcl->get_metadata()->get_hostPort()) +
                                        strlen(m_jcl->get_metadata()->get_mac()); // Indicates the size of the string

    for (i = 0; i < strlen(m_jcl->get_metadata()->get_mac()); i++)
        m_jcl->message[currentPosition++] = m_jcl->get_metadata()->get_mac()[i];

    for (i = 0; i < strlen(m_jcl->get_metadata()->get_hostPort()); i++)
        m_jcl->message[currentPosition++] = m_jcl->get_metadata()->get_hostPort()[i];
    ...
    m_jcl->message[currentPosition++] = strlen(Constants::dataTypeValue);     // Indicates the size of the String
    ...
    if (sensorNow) {
        ...
        m_jcl->get_client().flush();
// while (!m_jcl->get_client().available());
// while (m_jcl->get_client().available()) m_jcl->get_client().read();


        m_jcl->get_sensors()[pin]->set_lastExecuted(millis());

/*     if (m_jcl->getMQTTClient()->connected()){
      Serial.println(m_jcl->get_sensors()[pin]->get_sensorNickname());
      char mqttMessage[6];
      sprintf(mqttMessage, "%d", sensorValue);
      m_jcl->getMQTTClient()->publish(m_jcl->get_sensors()[pin]->get_sensorNickname(), mqttMessage);
    }*/
    }
...
bool Message::removeContextAction(bool is_acting) {
    uint16_t pos = 17;
    while (m_jcl->message[pos] != 74)
        pos++;
    pos++;
...
bool Message::setMetadata() {
    ...
    if (pointer != NULL) {
        ...
        for (int i = 0; i <= l; i++) {
            if (pointer[c] == ';' || i == l) {
                ...
                m_jcl->get_sensors()[atoi(pinN)] = s;
                //m_jcl->get_sensors()[atoi(pin)]->set_pin(pin);
                s->set_pin(pinN);

                m_jcl->get_sensors()[atoi(pinN)]->set_lastExecuted(0);
                char defaultName[] = "not set",
                        defaultDelay[] = "60",
                        defaultSensorSize[] = "1";
                m_jcl->get_sensors()[atoi(pinN)]->set_sensorNickname(
                        defaultName);   // In case the user didn't set the sensor name we use the default
                m_jcl->get_sensors()[atoi(pinN)]->set_delay(
                        defaultDelay);   // In case the user didn't set the delay we use the default 60
                m_jcl->get_sensors()[atoi(pinN)]->set_typeIO(Constants::CHAR_INPUT);   // the default is input sensor
                m_jcl->get_sensors()[atoi(pinN)]->set_type(0);   // default is generic sensor
                m_jcl->get_sensors()[atoi(pinN)]->set_sensorSize(defaultSensorSize);   // default is 1MB
                pos = 0;
                c++;
                numSensors++;
            } else {
    ...
    while ((pointer = strstr(pointer, "SENSOR_ALIAS_")) != NULL) {
        ...
        pin[pos] = '\0';
        if (!Sensor::validPin(atoi(pin)) || m_jcl->get_sensors()[atoi(pin)] == NULL) {
            return false;
        }
        s = m_jcl->get_sensors()[atoi(pin)];
        c++;
